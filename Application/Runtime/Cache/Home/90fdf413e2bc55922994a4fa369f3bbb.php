...
<script type="text/javascript" src="/tp/Public/js/check.js"></script>
<script type="text/javascript" src="/tp/Public/js/jquery-1.9.1.js"></script>
<script type="text/javascript" src="/tp/Public/js/blurSearch.js"></script>
<link rel="stylesheet" href="/tp/Public/css/blurSearch.css">
<script type="text/javascript">
	$ajaxUrl="<?php echo U('Fhk/ajaxAction','','');?>?id=";
	var searchUrl="<?php echo U('Fhk/index','','');?>";
</script>
<style type="text/css">
	table *{
		font-size: 10px;
...
<div class="admin-content">
	<ol class="am-breadcrumb">
		...
		<li><a href="<?php echo U('Fhk/index','','');?>/action/export">导出</a></li>
	</ol>
	<form action="<?php echo U('Fhk/index');?>" id="SearchForm"  method="post">
    <span>查找：</span>
	...
	<table class="am-table am-table-striped am-table-bordered am-text-sm am-table-hover">
		<tr class="am-success">
				...
				<th>
					放款金额
					<a href="<?php echo U('Fhk/index','','');?>/order/fk_money/role/asc" title="升序">
						<span class="am-icon-sort-amount-asc"></span>
					</a>
					<a href="<?php echo U('Fhk/index','','');?>/order/fk_money/role/desc" title="降序">
				...
				<th>
					放款日期
					<a href="<?php echo U('Fhk/index','','');?>/order/fk_date/role/asc" title="升序">
						<span class="am-icon-sort-amount-asc"></span>
					</a>
					<a href="<?php echo U('Fhk/index','','');?>/order/fk_date/role/desc" title="降序">
						<span class="am-icon-sort-amount-desc"></span>
					</a>
				</th>
				<th>合同状态</th>
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php echo number_format($vo['fk_money'],2,'.',',');?></td>
				<td>
					<?php if($vo["fk_type"] == 1): ?>信用
						<?php elseif($vo["fk_type"] == 2): ?>抵押
						<?php elseif($vo["fk_type"] == 3): ?>担保
						<?php elseif($vo["fk_type"] == 4): ?>其他<?php endif; ?>
				</td>
				<td><?php echo ($vo["hk_method"]); ?></td>
				<td><?php echo ($vo["limit_time"]); ?></td>
				<td><?php echo round($vo['rate']*100,2).'%';?></td>
				<td><?php echo (date('Y-m-d',$vo["fk_date"])); ?></td>
				<td>
					<?php if($vo['status'] == 1): ?>执行完毕
						<?php else: ?>执行中<?php endif; ?>
				</td>
				...
				<td><span title="<?php echo ($vo["info"]); ?>"><?php echo SubTitle($vo['info']);?></span></td>
				<td>
					<a href="<?php echo U('Fhk/scan','','');?>/id/<?php echo ($vo["agree_num"]); ?>">详情|</a>
					<a href="<?php echo U('Fhk/deleteAction','','');?>/id/<?php echo ($vo["agree_num"]); ?>" class="del">删除|</a>
					<a href="<?php echo U('Fhk/sk','','');?>/id/<?php echo ($vo["agree_num"]); ?>">收款明细</a>
				</td>
