...
<script src="/tp/Public/js/js.js" type="text/javascript"></script>
<link rel="stylesheet" type="text/css" href="/tp/Public/css/dksq.css" />
<script type="text/javascript">
	$(function(){
		$prev="<?php echo U('Fhk/preAction','','');?>";
	});
</script>

<div class="admin-content">
	<ol class="am-breadcrumb">
		<li><a href="<?php echo U('Fhk/pre','','');?>">审核</a></li>
	...
	<table class="am-table am-table-striped am-table-bordered am-text-sm am-table-hover">
		...
		<?php if(is_array($reInfo)): $i = 0; $__LIST__ = $reInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			<td><a href="<?php echo U('Fhk/scan','','');?>/id/<?php echo ($vo["agree_num"]); ?>"><?php echo ($vo["agree_num"]); ?></a></td>
			<td><?php echo ($vo["cname"]); ?></td>
			<td><?php echo ($vo["receive_term"]); ?></td>
			<td><?php echo (date("Y-m-d",$vo["receive_date"])); ?></td>
			<td><?php echo ($vo["hz_bj"]); ?></td>
			<td><?php echo ($vo["hzlx"]); ?></td>
			<td><?php echo ($vo["tqhkqx"]); ?></td>
			<td><?php echo ($vo["wyj"]); ?></td>
			<td><?php echo ($vo["fx"]); ?></td>
			<td><button title="设置" class="preBtn am-sm-btn am-btn-default">设置</button></td>
