...
<div class="admin-content">
	<ol class="am-breadcrumb">
		<li><a href="<?php echo U('Fhk/clientView','','');?>">客户账款</a></li>
		<li class="am-active">列表</li>
		<li><a href="<?php echo U('Fhk/clientView','','');?>/action/export">导出</a></li>
	</ol>
	<form action="<?php echo U('Fhk/clientView');?>" method="post">
    <span>查找姓名：</span>
	...
	<table class="am-table am-table-striped am-table-bordered am-text-sm am-table-hover">
		<tr class="am-success">
			...
			<th>
				结算金额
				<a href="<?php echo U('Fhk/clientView','','');?>/order/ResultMoney/role/asc" title="升序">
					<span class="am-icon-sort-amount-asc"></span>
				</a>
				<a href="<?php echo U('Fhk/clientView','','');?>/order/ResultMoney/role/desc" title="降序">
		...
		<?php if(is_array($out)): $i = 0; $__LIST__ = $out;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				<td><a href="<?php echo U('Fhk/scan','','');?>/id/<?php echo ($vo["agree_num"]); ?>"><?php echo ($vo["agree_num"]); ?></a></td>
				<td><?php echo ($vo["cname"]); ?></td>
				<?php if(is_array($userinfo)): $i = 0; $__LIST__ = $userinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$uinfo): $mod = ($i % 2 );++$i; if($uinfo['id'] == $vo['saler']): ?><td><?php echo ($uinfo["xingming"]); ?></td><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				<td><?php echo number_format($vo['ResultMoney'],2,'.',',');?></td>
				<td><?php echo number_format($vo['AgreeHk'],2,'.',',');?></td>
				<td><?php echo number_format($vo['Tqqx'],2,'.',',');?></td>
				<td><?php echo number_format($vo['Wyj'],2,'.',',');?></td>
				<td><?php echo number_format($vo[Fx],2,'.',',');?></td>
				<td><?php echo number_format($vo['YhJe'],2,'.',',');?></td>
				<td><?php echo ($vo["Yhzb"]); ?></td>
				<td><?php echo floor($vo['HkJs']);?></td>
				<td><?php echo number_format($vo['YsJe'],2,'.',',');?></td>
				<td><?php echo ($vo["Yszb"]); ?></td>
				<td><?php echo floor($vo['YsJs']);?></td>
				<td><?php echo number_format($vo['DsJe'],2,'.',',');?></td>
				<td><?php echo ($vo["Dszb"]); ?></td>
				<td><?php echo floor($vo['DsJs']);?></td>
			</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php if($Sum['CCount'] != 0): ?><tr>
				<td>统计：<?php echo ($Sum['CCount']); ?></td>
				<td><?php echo ($Sum["CCount"]); ?></td>
				<td class="ulnav">浏览..
					<div class="hide">
					<?php if(is_array($SaCount)): $i = 0; $__LIST__ = $SaCount;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sa): $mod = ($i % 2 );++$i; if(is_array($userinfo)): $i = 0; $__LIST__ = $userinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$uinfo): $mod = ($i % 2 );++$i; if($uinfo['id'] == $sa['saler']): echo ($uinfo["xingming"]); ?>：<?php echo ($sa["count"]); ?><br/><?php endif; endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
					</div>
				</td>
				<td><?php echo number_format($Sum['ResultMoneySum'],2,'.',',');?></td>
				<td><?php echo number_format($Sum['AgreeHkSum'],2,'.',',');?></td>
				<td><?php echo number_format($Sum['TqqxSum'],2,'.',',');?></td>
				<td><?php echo number_format($Sum['WyjSum'],2,'.',',');?></td>
				<td><?php echo number_format($Sum['FxSum'],2,'.',',');?></td>
				<td><?php echo number_format($Sum['YhJe'],2,'.',',');?></td>
				<td><?php echo round($Sum['YhJe']/$Sum['ResultMoneySum']*100,2).'%';?></td>
				<td><?php echo floor($Sum['HkJsSum']);?></td>
				<td><?php echo number_format($Sum['YsJe'],2,'.',',');?></td>
				<td><?php echo round($Sum['YsJe']/$Sum['ResultMoneySum']*100,2).'%';?></td>
				<td><?php echo floor($Sum['YsJsSum']);?></td>
				<td><?php echo number_format($Sum['DsJe'],2,'.',',');?></td>
				<td><?php echo round($Sum['DsJe']/$Sum['ResultMoneySum']*100,2).'%';?></td>
				<td><?php echo floor($Sum['DsJsSum']);?></td>
			</tr><?php endif; ?>
	</table>
