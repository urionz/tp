...
<div class="admin-content">
	...
	<table class="am-table am-table-bordered am-table-striped am-text-sm ">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><?php if($vo["status"] == 0 ): ?>禁用
					<?php elseif($vo["status"] == 1 ): ?>正常<?php endif; ?>
				</td>
				<td><?php echo ($vo["info"]); ?></td>
				<td>
					<a href="<?php echo U('Auth/groupCk','','');?>?sq=<?php echo ($vo["id"]); ?>">查看/修改权限</a>
					|<a href="<?php echo U('Auth/accessList','','');?>?sq=<?php echo ($vo["id"]); ?>">成员管理</a>
				</td>
