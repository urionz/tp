...
<div class="admin-content">
	<ol class="am-breadcrumb">
		<li><a href="<?php echo U('Fhk/TcList','','');?>">提成统计</a></li>
		<li class="am-active">列表</li>
	</ol>
	<div class="am-form-group">
    
    <form action="<?php echo U('Fhk/TcList');?>" method="post">
    <label>开始日期</label>
	...
	<table class="am-table am-table-striped am-table-bordered am-text-sm am-table-hover">
		...
		<?php if(is_array($ReAgreeInfo)): $i = 0; $__LIST__ = $ReAgreeInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                <?php if(is_array($userinfo)): $i = 0; $__LIST__ = $userinfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$uinfo): $mod = ($i % 2 );++$i; if($uinfo['id'] == $vo['saler']): ?><td><a href="<?php echo U('Fhk/TcDetail','','');?>/id/<?php echo ($vo["id"]); ?>"><?php echo ($uinfo["xingming"]); ?></a></td><?php endif; endforeach; endif; else: echo "" ;endif; ?>
				<td><?php echo number_format($vo['tc_sum'],2,'.',',');?></td>
